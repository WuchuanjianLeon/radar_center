...
void STE_Ct(void)//舵机校准PWM控制函数
{
	if(sw[0]&&ste_status1==STE_NORMAL&&flagtest1==0)
	{
	  ...
	  if(duty_cycle4>=ST_THRE_MAX||duty_cycle4<=ST_THRE_MIN) {chflag1=FALSE;duty_cycle4=1500;duty_step4=-duty_step4;}
	  
	  if(ste_flagt[0]==0&&chflag1){duty_ste_flag[0]++;}
	  else{duty_ste_flag[0]=0;}
   ...
   if(sw[1]&&ste_status2==STE_NORMAL&&flagtest2==0)
	 {
	   ...
	   if(duty_cycle3>=ST_THRE_MAX||duty_cycle3<=ST_THRE_MIN) {chflag2=FALSE;duty_cycle3=1500;duty_step3=-duty_step3;}
	   
	   if(ste_flagt[1]==0&&chflag2){duty_ste_flag[1]++;}
	   else{duty_ste_flag[1]=0;}
	...
	if(sw[2]&&ste_status3==STE_NORMAL&&flagtest3==0)
	{
	  ...
	  if(duty_cycle2>=ST_THRE_MAX||duty_cycle2<=ST_THRE_MIN) {chflag3=FALSE;duty_cycle2=1500;duty_step2=-duty_step2;}

	  if(ste_flagt[2]==0&&chflag3){duty_ste_flag[2]++;}
	  else{duty_ste_flag[2]=0;}
   ...
   if(sw[3]&&ste_status4==STE_NORMAL&&flagtest4==0)
   {
	   ...
	   if(duty_cycle1>=ST_THRE_MAX||duty_cycle1<=ST_THRE_MIN) {chflag4=FALSE;duty_cycle1=1500;duty_step1=-duty_step1;}
	   
	   if(ste_flagt[3]==0&&chflag4){duty_ste_flag[3]++;}
	   else{duty_ste_flag[3]=0;}
	...
	if(SReadAddress==16)
	{
		...
		if((acc_scale_zero.flag&0x00ff)==ACC_FLAG_FLASH)
		{
          ...
          if(fabs(acc_scale_zero.ax_gain-1.0)>0.1 ||
		  	...
		  	{
		  	  acc_scale_zero.ax_gain=1.0;acc_scale_zero.ay_gain=1.0;acc_scale_zero.az_gain=1.0;
			  acc_scale_zero.ox_offset=0.0;acc_scale_zero.oy_offset=0.0;acc_scale_zero.oz_offset=0.0;
			}
